...
protocol FiniteStateMachine
{
    ...
    var current_state: StateMachine.State { get }
    var initial_state: StateMachine.State { get }

    func activate()
    func can_fire(event: StateMachine.Event) -> Bool
    func fire(event: StateMachine.Event, user_info: StateMachine.Payload) throws
}

public extension StateMachine
{
    typealias Payload = [String: Any]
    typealias TransitionContextAction<Context> = (_ context: Context, _ transition: Transition?) -> Void
...
public class StateMachine: FiniteStateMachine
{
    ...
    public func state(named: String) -> Result<State, Error>
    {
        ...
        {
            result = Result.failure(Error.not_found)
        }

        return result
    }

    public func event(named: String) -> Result<Event, Error>
    {
    ...
    public func fire(event: Event, user_info: Payload = [:]) throws
    {
        ...
        if !can_fire(event: event)
        {
            ...
            throw Error.cannot_fire(message: message)
        }
        else
        {
            // Nothing to do.
           ...
           !should_file(event, transition)
        {
            ...
            throw Error.declined(message: message)
        }
        else
        {
