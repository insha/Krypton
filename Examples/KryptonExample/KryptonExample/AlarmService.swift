...
extension AlarmService
{
    enum Events: String
    {
        ...
        {
            try fsm.event(named: rawValue).get()
        }
    }
...
extension AlarmService
{
    ...
    private struct StatesAndEvents
    {
    ...
    private static func states_and_events() throws -> StatesAndEvents
    {
        let state_transitions = State.Context(
