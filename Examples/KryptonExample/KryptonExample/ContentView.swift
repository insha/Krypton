    ...
    var body: some View
    {
        VStack(alignment: .leading)
        {
            ...
            HStack(spacing: 20)
            {
                ...
                Button(action: { service.arm() }, label: { Text("Arm") })
                    .disabled(!service.can_trigger(event: .arm))

                Button(action: { service.disarm(code: alarm_code) }, label: { Text("Disarm") })
                    .disabled(!service.can_trigger(event: .disarm))

                Button(action: { service.reset(code: alarm_code) }, label: { Text("Reset") })
                    .disabled(!service.can_trigger(event: .reset))

                Button(action: { service.panic() }, label: { Text("Panic") })
